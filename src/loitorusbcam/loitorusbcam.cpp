...
void *cam1_capture(void*)
{
    ...
    while(!shut_down_tag)
    {
		...
		if(gImg1_SysTime[gFrameCam1].tv_usec<0){gImg1_SysTime[gFrameCam1].tv_usec+=1000000;gImg1_SysTime[gFrameCam1].tv_sec-=1;}
...
void *cam2_capture(void*)
{
    ...
    while(!shut_down_tag)
    {
		...
		if(gImg2_SysTime[gFrameCam2].tv_usec<0){gImg2_SysTime[gFrameCam2].tv_usec+=1000000;gImg2_SysTime[gFrameCam2].tv_sec-=1;}
                fTimeuse = (endTime.tv_sec - visensor_startTime.tv_sec) + 0.000001*(endTime.tv_usec - visensor_startTime.tv_usec);
                fTimeuse-=time_offset;	// 真正的图像拍摄时间
